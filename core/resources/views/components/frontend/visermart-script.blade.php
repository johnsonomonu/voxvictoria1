<script>
    ...
    function addToCartClickHandler() {
        ...
        let quantityField = $(this).closest('.product-details, .product-add-to-cart, .product-card, .product-content, .product-quick-view-btn').find('[name=quantity]');
        if (!quantityField.length) {
            quantityField = $(this).parent().find('[name=quantity]');
        }
    ...
    function buyNowClickHandler() {
        ...
        $.post(addToCartUrl, data).done((response) => {
            if (response.status == 'success') {
                ...
                setCartCount(response.cartItemCount);
                setCartSubtotal(response.cartSubtotal);
                window.location.href = "{{ route('user.checkout.shipping.info') }}";
                return;
            }
            notify(response.status, response.message);
        });
